  ...
  toMatchOutput(received: Received, output) {
    ...
    if (fs.existsSync(output)) {
      const expected = buildText(
        readdir(output).map((filename: string) => ({
          filename: filename,
          content: eol.lf(sander.readFileSync(path.join(output, filename)).toString()),
        }))
      );
        ...
        if (this.equals(actual, expected)) {
          ...
          if (snapshotState._updateSnapshot === 'all') {
            ...
            received.forEach(file => {
              sander.writeFileSync(path.join(output, file.filename), file.content);
            });
            ...
            return {
              ...
              message: () =>
                `Received content ${chalk.red(
                  "doesn't match"
                )} the output ${output}.\n\n${diff(expected, actual, options.diff)}`,
            };
          }
        }
      }
      ...
      if (
        ...
        return {
          pass: true,
          message: () =>
            `The output file ${chalk.blue(output)} ${chalk.bold.red("doesn't exist")}.`,
        };
      }
    }
  },
